// VERSION 20 - With a certain parameter being adjusted for each mini-match.
...
//    void set_future_positions(const vector<unique_ptr<position>>& future_positionsP);
    ...
    static double parameter_being_adjusted;
...
double position::parameter_being_adjusted = 6.0;
                                                ...
                                                vector<vector<char>>& copy_board, vector<coordinate_and_value>& recorder) const
{
    ...
    const double small_amount = parameter_being_adjusted; // NOTE: if you change this to a double, recorder (and its parent in prev. function) should store coordinate and DOUBLE.
